...
def spider_id(file_path='new_img'):
    page=0
    while page<=10:
        ...
        http = {"https": 'https://' + ip}
        url = "http://music.baidu.com/songlist/tag/全部?orderType=0&offset="+str(page*20)
        print url
        try:
...
def spider_info(id2,size=16, chars=string.ascii_letters + string.digits):
    ...
    ms = db['new_singleinfo']
    ms.insert(data)
    if len(dict) < 1:
        idm = str(id2)
        id3 = db.new_music.find_one({'songid':idm})['_id']
        id4 = db.new_singleinfo.find_one({'titleid': idm})['_id']
        db.new_music.remove(id3)
        db.new_singleinfo.remove(id4)
